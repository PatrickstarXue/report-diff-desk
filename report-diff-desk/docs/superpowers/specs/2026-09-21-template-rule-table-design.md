...
**锚点词可配（2026-09-21 追加）**：锚点词默认「项目」，用户可在「表样核对」工具栏改成自己报表标签区左上角的文字（如「期限」），**多个用「、」分隔**、按填写顺序尝试、每词左上优先；比较前两端都去掉全部空白，所以「项 目」也能命中「项目」。候选存进 `AlignConfig.anchors`（`userData/template-align.json` 的可选字段，旧文件缺席即用默认），这样多个报表族可以一次列全。锚点词不是该报表的标签名时，该侧照常走下面的降级解析。
