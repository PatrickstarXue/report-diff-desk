...
3. **结果展示**：环比结果概览（全局指标 + 每对文件卡片）+ 变动明细表（红涨绿跌橙新增）+ 报表环比网格（黄底）+ 行点击跳转定位
4. **口径查询**：导入两列映射表（指标名、口径说明），点击任意单元格按文本精确命中显示口径
5. **口径文档**：打开 Word(.docx)/PDF/TXT 原文翻阅，支持关键字过滤、字号调节
6. **新旧表数据比对**：左右各选一套报表（R 系列 / NR 系列，一个 zip 内可有多个表号），按表号配对、按规则值等值配对比对。规则值形如 `行标签_列标签`（种子由锚点解析生成，锚点词可配），配不上的在左右两张可编辑规则表里改成一致；规则表按表样对持久化，下次直接复用
7. **导出**：Excel 明细（带行填充色与百分比格式）+ 自包含 HTML 报告（内嵌样式，断网可开）
