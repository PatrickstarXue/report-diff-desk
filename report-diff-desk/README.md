...
3. 超大表（>5 万行）网格渲染可能偏慢
4. **导出保格式**：导出固定为「一个文件、上期/本期两个 sheet」。源是 .xlsx 时与该表一致（字体、居中、边框、列宽、行高、合并全部保留）；源是 .xls 时只能重建——数值格式、列宽、行高、合并带得过去，**字体、居中、边框保留不了**（exceljs 读不了 biff8，BIFF8 也写不出来）。把源另存为 .xlsx 再上传即可完全一致
